...
class cstudent extends cTable {
	...
	function __construct() {
		...
		// usn
		$this->usn = new cField('student', 'student', 'x_usn', 'usn', '`usn`', '`usn`', 200, -1, FALSE, '`usn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// sem
		$this->sem = new cField('student', 'student', 'x_sem', 'sem', '`sem`', '`sem`', 3, -1, FALSE, '`sem`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// gender
		$this->gender = new cField('student', 'student', 'x_gender', 'gender', '`gender`', '`gender`', 200, -1, FALSE, '`gender`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->gender->Sortable = TRUE; // Allow sort
		$this->fields['gender'] = &$this->gender;

		// backlogs
		$this->backlogs = new cField('student', 'student', 'x_backlogs', 'backlogs', '`backlogs`', '`backlogs`', 3, -1, FALSE, '`backlogs`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->backlogs->Sortable = TRUE; // Allow sort
		$this->backlogs->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['backlogs'] = &$this->backlogs;

		// fid
		$this->fid = new cField('student', 'student', 'x_fid', 'fid', '`fid`', '`fid`', 3, -1, FALSE, '`fid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('usn', $rs))
				ew_AddFilter($where, ew_QuotedName('usn', $this->DBID) . '=' . ew_QuotedValue($rs['usn'], $this->usn->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (is_null($this->usn->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@usn@", ew_AdjustSql($this->usn->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "usn:" . ew_VarToJson($this->usn->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->usn->CurrentValue)) {
			$sUrl .= "usn=" . urlencode($this->usn->CurrentValue);
		} else {
	...
	function LoadListRowValues(&$rs) {
		...
		$this->backlogs->setDbValue($rs->fields('backlogs'));
	...
	function RenderEditRow() {
		...
		$this->backlogs->EditValue = $this->backlogs->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->backlogs->Exportable) $Doc->ExportCaption($this->backlogs);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->backlogs->Exportable) $Doc->ExportField($this->backlogs);
